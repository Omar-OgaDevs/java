  ...
  private void performRemoval(ListItem item, ListItem parent) {
    ...
    if (item.next() == null) {
      ...
      if (parent.next() == item) {
        ...
        //parent must be item, which means we were looking at
      ...
      if (leftmostParent == item) {
        //there was no leftmost node, so current points tho the smallest
        //node (the one that now must be deleted)
        item.setNext(current.next());
      } else {
        //set the smallest node's parent to poiny to
