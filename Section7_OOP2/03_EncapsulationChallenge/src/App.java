/*
Create a class and demonstrate proper encapsulation techniques
The class will be called Printer
It will simulate a real Computer Printer
It should have fields for the toner Level, number of pages
printed, and also whether its a duplex printer
Add methods to fill up the toner (up to a maximum of 100%)
Another method to simulate printing a page (which should increase
the number of pages printed)
Decide on the scope, whether to use constructors, and anything
else you think is needed.
*/

public class App {
    public static void main(String[] args) throws Exception {
        
        Printer printer = new Printer(50, false);
        //Using encapsulation technique on the getter
        System.out.println("Initial paga count = " + printer.getPagesPrinted());
        int pagesPrinted = printer.printPages(4);
        System.out.println("Pages printed was " + pagesPrinted + " new total print count = " + printer.getPagesPrinted());

        pagesPrinted = printer.printPages(2);
        System.out.println("Pages printed was " + pagesPrinted + " new total print count = " + printer.getPagesPrinted());
    }
}
